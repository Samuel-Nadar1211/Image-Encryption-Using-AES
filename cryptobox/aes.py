...
def keyExpansion(key):
    global KEY_SIZE, ROUNDS
    KEY_SIZE = len(key) // 4
    ROUNDS = KEY_SIZE + 6
    w = [[0] * 4 for _ in range(4 * (ROUNDS + 1))]
    ...
    for i in range(KEY_SIZE, 4 * (ROUNDS + 1)):
        temp = []
        for j in range(4):
            temp.append(w[i - 1][j])
        ...
        if i % KEY_SIZE == 0:
            g(temp, i // KEY_SIZE)
        elif KEY_SIZE > 6 and i % KEY_SIZE == 4:  # For AES-256
            for j in range(4):
                temp[j] = s_box[temp[j]]
